...
*/
void ACL_begin(PmodACL* InstancePtr,  u32 SPI_Address)
{
...
**
*/
void SetRegisterBits(PmodACL* InstancePtr, u8 reg, u8 mask, bool fValue){
	u8 regval;
...
**
*/
u8 GetRegisterBits(PmodACL* InstancePtr, u8 bRegisterAddress, u8 bMask)
{
	u8 bRegValue;
...
**				u16 uiReading	- the 2 bytes containing the reading (in fact only 10 bits are used).
**
**
**        Return Values:
...
**
*/
float ConvertReadingToValueG(PmodACL* InstancePtr, u16 uiReading)
{
...
**        Parameters:
**				u8 bGRange	- the parameter specifying the g range. Can be one of the parameters in the following list
...
**
**        Description:
**				This function is the main function used for acceleration reading, providing the 3 current acceleration values in
...
**					-	For each of the three axes, converts the 10-bit value to the value expressed in, considering the currently selected g range
...
void ReadAccelG(PmodACL* InstancePtr, float *dAclXg, float *dAclYg, float *dAclZg) {
	u16 rgwRegVals[3];
	ACL_ReadSpi(InstancePtr, ACL_ADR_DATAX0, (u8 *)rgwRegVals, 6);
...
**        Parameters:
**        		PmodACL *InstancePtr	- the PmodACL object to communicate with
**				u16 *iAclX	- the output parameter that will receive acceleration on X axis - 10 bits signed value
**				u16 *iAclY	- the output parameter that will receive acceleration on Y axis - 10 bits signed value
**				u16 *iAclZ	- the output parameter that will receive acceleration on Z axis - 10 bits signed value
**
**
**        Return Values:
**                void
...
**
*/
void ReadAccel(PmodACL* InstancePtr, u16 *iAclX, u16 *iAclY, u16 *iAclZ)
{
	u16 rgwRegVals[3];
	ACL_ReadSpi(InstancePtr, ACL_ADR_DATAX0,(u8 *)rgwRegVals, 6);
	*iAclX = rgwRegVals[0];
...
**        Parameters:
**        		PmodACL *InstancePtr	- the PmodACL object to communicate with
**				u8 bGRangePar	- the parameter specifying the g range. Can be one of the parameters from the following list:
**					0	PAR_GRANGE_PM2G	Parameter g range : +/- 2g
**					1	PAR_GRANGE_PM4G	Parameter g range : +/- 4g
**					2	PAR_GRANGE_PM8G	Parameter g range : +/- 8g
**					3	PAR_GRANGE_PM16G Parameter g range : +/- 16g
...
**
*/
void SetGRange(PmodACL *InstancePtr, u8 bGRangePar)
{
	u8 bResult;
...
**        Return Values:
**        		PmodACL *InstancePtr	- the PmodACL object to communicate with
**                u8 - the value specifying the g Range parameter. Can be one of the values from the following list
**					0	PAR_GRANGE_PM2G	Parameter g range : +/- 2g
**					1	PAR_GRANGE_PM4G	Parameter g range : +/- 4g
**					2	PAR_GRANGE_PM8G	Parameter g range : +/- 8g
**					3	PAR_GRANGE_PM16G Parameter g range : +/- 16g
...
**
*/
u8 GetGRange(PmodACL* InstancePtr)
{
	u8 bVal = (GetRegisterBits(InstancePtr, ACL_ADR_DATA_FORMAT, MSK_DATA_FORMAT_RANGE1) << 1) + GetRegisterBits(InstancePtr, ACL_ADR_DATA_FORMAT, MSK_DATA_FORMAT_RANGE0);
	return bVal;
}
...
**        Parameters:
**				PmodACL* InstancePtr	- the PmodACL object to communicate with
**				u8 bAxisParam - byte indicating the axis whose offset will be set. Can be one of:
...
**
**        Description:
**				This function sets the specified axis offset, the value being given in The accepted argument values are between -2g and +2g.
...
**
*/
void SetOffsetG(PmodACL* InstancePtr, u8 bAxisParam, float dOffset)
{
	u8 bOffsetVal = (u8)(dOffset/(float)ACL_CONV_OFFSET_G_LSB);
	switch (bAxisParam)
	{
		case PAR_AXIS_X:
			ACL_WriteSpi(InstancePtr, ACL_ADR_OFSX, (u8 *)&bOffsetVal, 1);
			break;
		case PAR_AXIS_Y:
			ACL_WriteSpi(InstancePtr, ACL_ADR_OFSY, (u8 *)&bOffsetVal, 1);
			break;
		case PAR_AXIS_Z:
			ACL_WriteSpi(InstancePtr, ACL_ADR_OFSZ, (u8 *)&bOffsetVal, 1);
			break;
	}
}

float GetOffsetG(PmodACL* InstancePtr, u8 bAxisParam)
{
	u8 bOffsetVal;
	float dResult;
	switch (bAxisParam)
	{
		case PAR_AXIS_X:
			ACL_ReadSpi(InstancePtr, ACL_ADR_OFSX, (u8 *)&bOffsetVal, 1);
			break;
		case PAR_AXIS_Y:
			ACL_ReadSpi(InstancePtr, ACL_ADR_OFSY, (u8 *)&bOffsetVal, 1);
			break;
		case PAR_AXIS_Z:
			ACL_ReadSpi(InstancePtr, ACL_ADR_OFSZ, (u8 *)&bOffsetVal, 1);
			break;
	}
...
**        Parameters:
**        		PmodACL *InstancePtr	- the PmodACL object to communicate with
**				u8 bAxisInfo - Parameter specifying axes orientation. Can be one of the following:
...
**
*/
void CalibrateOneAxisGravitational(PmodACL* InstancePtr, u8 bAxisInfo)
{
		// perform calibration
